    ...
    def parse_l_message(self, message):
        ...
        while pos < len(data):
            ...
            if device and (self.is_thermostat(device) or self.is_wallthermostat(device)):
                ...
                device.linkStatusError = (bits2 >> 6) & 1
                device.lowBattery = (bits2 >> 7) & 1
                device.statusInitialized = (bits1 >> 1) & 1
                device.errorStatus = (bits1 >> 3) & 1
                device.informationValid = (bits1 >> 4) & 1
